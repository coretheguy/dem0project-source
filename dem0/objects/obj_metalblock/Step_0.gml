...
if place_meeting(x, y + 15, obj_player) && obj_player.state = 102 && obj_player.mach2 = 100
{
    instance_destroy()
    tile_layer_delete_at(1, x, y)
    audio_sound_gain(sfx_break, 0.7, 0)
...
if ((place_meeting((x + 1), y, obj_chasingmonsterdark) || place_meeting((x - 1), y, obj_chasingmonsterdark)))
{
    instance_destroy()
    audio_sound_gain(sfx_break, 0.7, 0)
    if (!audio_is_playing(sfx_break))
