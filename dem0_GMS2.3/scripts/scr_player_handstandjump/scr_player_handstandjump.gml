function scr_player_handstandjump() {
	...
	//}
	if ((place_meeting(x, (y + 1), obj_collisionparent) && (vsp > 0)) && ((sprite_index == spr_player_fall2)))
	{
	    if key_attack
	        landAnim = 0
	    state = 0
	...
	if key_down && !place_meeting(x, y + 1, obj_collisionparent) && sprite_index = spr_player_fall2
	{
		...
		scr_sound(sfx_supermove)
	}
	


	if place_meeting(x, (y + 1), obj_collisionparent) && (key_jump) && (sprite_index == spr_player_fall2)
	{
	    stompAnim = 0
		vsp = -11
	    jumpAnim = 1
